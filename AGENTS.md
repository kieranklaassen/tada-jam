...
- **Showcases are not cartridges.** An owner-approved showcase (so far only Alien Frontier) lives in `showcase/<key>/`, is listed apart from the games, and is exempt from the cartridge rules (age band, wordless, kid-side mechanics) but never from the egress and built-asset checks; `test/showcase.test.ts` holds that line.
