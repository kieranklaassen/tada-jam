...
describe('steer and advance', () => {
  it('turns toward the target at a limited rate', () => {
    const mover: Mover = { x: 10, z: 15, heading: 0 }
  ...
  it('veers away from a friend in the way', () => {
    const mover: Mover = { x: 0, z: 20, heading: Math.PI / 2 }
    const friend: Mover = { x: 8, z: 21.5, heading: 0 }
  ...
  it('never walks onto the turntable or off the bench', () => {
    const mover: Mover = { x: TURNTABLE.x, z: TURNTABLE.z + 30, heading: Math.PI }
    for (let i = 0; i < 200; i++) {
      advance(mover, 1)
      expect(insideWalk(mover, 0.9)).toBe(true)
    }
    const edge: Mover = { x: 20, z: 0, heading: Math.PI / 2 }
    for (let i = 0; i < 50; i++) advance(edge, 1)
    expect(insideWalk(edge, 0.9)).toBe(true)
  })

  it('knows when it has arrived and when friends meet', () => {
    expect(arrived({ x: 0, z: 20, heading: 0 }, { x: 1, z: 21 })).toBe(true)
    expect(arrived({ x: 0, z: 20, heading: 0 }, { x: 10, z: 20 })).toBe(false)
    expect(meeting({ x: 0, z: 20, heading: 0 }, { x: 15, z: 20, heading: 0 })).toBe(true)
    expect(meeting({ x: -40, z: 20, heading: 0 }, { x: 15, z: 20, heading: 0 })).toBe(false)
  })
})
