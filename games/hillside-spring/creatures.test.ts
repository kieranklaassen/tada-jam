...
describe('Presence', () => {
  ...
  it('the tanuki walks over to a spot at its own end of the terraces, and goes round for one at the other end', () => {
    const tanuki = new Presence('tanuki')
    tanuki.settle({ c: 2, r: 1 }, 0)
    tanuki.update(1, { c: 1, r: 3 })
    expect(tanuki.phase).toBe('arriving')
    tanuki.settle({ c: 2, r: 1 }, 10)
    tanuki.update(11, { c: 5, r: 1 })
    expect(tanuki.phase).toBe('leaving')
    tanuki.update(11 + TRAVEL_SECONDS.tanuki + 0.01, { c: 5, r: 1 })
    expect(tanuki.phase).toBe('away')
    tanuki.update(11 + TRAVEL_SECONDS.tanuki + 0.02, { c: 5, r: 1 })
    expect(tanuki.phase).toBe('arriving')
    expect(tanuki.spot).toEqual({ c: 5, r: 1 })
  })
})
