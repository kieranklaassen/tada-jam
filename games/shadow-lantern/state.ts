...
export type SkyCreature = { kind: CreatureKind; slot: number }
 ...
 * The screen starts clean; the first drag brings a shadow sweeping in.
 */
...
function readSky(raw: unknown): SkyCreature[] {
  ...
  for (const item of raw) {
    if (!item || typeof item !== 'object') continue
    const { kind, slot } = item as Record<string, unknown>
...
export function serialize(state: TheatreState): TheatreState {
  ...
  return {
    ...
    sky: state.sky.map((creature) => ({ kind: creature.kind, slot: creature.slot })),
  }
}
...
export function wakeCreature(state: TheatreState): { arrived: SkyCreature; departed: SkyCreature | null } {
  ...
  const arrived = { kind: state.sleeping, slot }
