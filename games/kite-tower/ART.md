...
- **60 fps on a mid-range iPad.** 38–39 draw calls, one full-screen multiply pass with no render target, no shadow maps, DPR capped at 2. The atlas is built once per page load; the geometry is built once when the game opens (never per piece, render or frame) and freed when it closes. On the merged production build, frame CPU p95 at Chromium 4× is 6.5–7.0 ms on auto and 6.4–7.9 ms at the minimal tier. At 6× it is 8.9–11.3 and 9.7–10.8 ms, over the 8 ms target on the VM that took the final numbers; the same code read 7.4–7.5 and 6.4–8.0 on pass 30's faster VM. WebKit on auto runs at 1.19× Pebble Table's fps in a clean same-session round. The VM has no GPU, so the fps it reports is software fill rate, and the top tier pinned (DPR 2) is fill-bound there: 0.60–0.63× Pebble Table on WebKit. **No physical iPad was measured**, so the 60 fps line still needs a device or the true-GPU pass on a Mac. Full table and method: "Final numbers" in `REFINEMENT.md`.
