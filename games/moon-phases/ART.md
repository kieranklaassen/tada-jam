...
- **Adaptive quality** ([`quality.ts`](quality.ts)), with the same governor as Bad Neighbours: 40-frame (or 2 s) windows, the longest frame left out, a step up only after six clean windows in which nine frames in ten did under 8 ms of work, a fresh upgrade on probation (one bad 12-frame window takes it back and makes it a ceiling), touch devices one tier down. `?tier=N` pins a tier.

  | Tier | Pixel ratio cap | Post chain | Window refreshed | Frosted glass |
  | --- | --- | --- | --- | --- |
  | 0 | 2 | depth of field, bloom, film grade | every frame | yes |
  | 1 | 1.5 | bloom | every 2nd frame | yes |
  | 2 | 1.25 | output pass only | every 2nd frame | no |
  | 3 | 1 | output pass only | every 4th frame | no |
...
- Earth and the moon are hit-tested as spheres, the pins and the day dial write styles only when they have moved visibly, and the audio context is built, suspended, during the opening flight.
