...
- **Architecture is one warm family.** Coral stone, rose accents, lilac plinths, cream trims, all flat shaded: tops brightest, left faces mid, right faces darker, undersides tinted violet. No textures anywhere.
- **The hue split is the whole point.** The geometric style's weakness is that everything is one pastel family and nothing says "touch me". Here everything the child uses has its own hue, and nothing decorative borrows it:
  ...
  - everything that moves is **sunflower yellow** with indigo hubs: turn wheels, slide grips, and the bird (who is a moving bridge);
...
- **Calm dusk.** Slow drifting motes, faint distant spires, lit windows. Nothing flashes or nags.
...
| Stone / rose / lilac / trim | `#f0a08c` / `#e7877c` / `#b3a0cf` / `#f6e6d2` | Architecture only |
...
| Handles and bird | `#ffc53d`, bird `#ffcb52` / wings `#f2a33a`, hubs `#40357a` | "You can move this" |
...
- **Adaptive quality** ([`tiers.ts`](tiers.ts)): four tiers step DPR 2 → 1.5 → 1.25 → 1 and drop motes, spires, sky dither and halos. A slow median frame steps down; stepping back up needs a calm stretch that doubles after every drop. `?tier=N` pins a tier; `?fps=1` shows a wordless frame-time bar graph; `window.__jamPerf` exposes per-frame CPU time for the jam's perf probe.
