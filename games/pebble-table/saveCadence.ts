// When to hand the table to ctx.storage (KTD10). The storage layer debounces
// and the shell flushes on put-away; this only makes sure the newest table,
// including a stone mid-drag, is always what the last save() carried.
...
export class SaveCadence {
  ...
  /** The table changed but is still moving (rolling stones); save once it settles. */
  markDirty(): void {
    this.dirty = true
  }

  /** The table came to rest, attention dropped, or the page is hiding: save any pending change. */
