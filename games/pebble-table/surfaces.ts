...
/** The knots a pan's ropes are tied with, pressed into the beam end above and the rolled rim below (cm). */
export const ROPE_KNOT = { radius: 0.9, press: 0.35 }

/**
 * How far (cm) the rolled rim of a pan of radius `r` (cm) reaches out from its
 * middle, and how high it rises above the pan's hanging point, and the rope
 * knots tied on it.
 */
export function panRimReach(r: number): { out: number; top: number; knots: number } {
  const reach = r * PAN_ROLL.radius
  const top = PAN_ROLL.y + reach * PAN_ROLL.tube
  return { out: reach * (1 + PAN_ROLL.tube), top, knots: top + ROPE_KNOT.radius * 2 - ROPE_KNOT.press }
}
