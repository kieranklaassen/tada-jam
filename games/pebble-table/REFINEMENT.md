...
**Scale cost pass.** Profiling a seeded Honest Scale play in Node (the controller's step and `sunk` for every part, as the view asks) found cannon placing every shape of a stick or shell again for each shape it is tried against, a pan's separating-axis tests run against stones whose boxes were apart, and `sunk` finding every pair afresh each frame. Each body's shapes are now placed once per pose, two shapes are tried only if their world bounds meet, and `sunk` reuses a pair's contacts until one of the two moves; the pour still lands bit for bit where trying every shape does. Profiling also found that a pan asleep at rest was met by the broadphase up to 2.3 cm from where it hung, so a part dropped in as the beam tipped met it late; the pan's bounds are now marked after every direct move, which costs back some of what the cuts saved. A seeded frame-budget test counts the busiest scale play's work: under 2.2 cannon steps, 160 shape tests and 14 of `sunk`'s a frame, and 200 contacts at most. In Node at 1× the play's mean frame is 0.98 ms (before this pass 0.99, `main` 0.54). In Chromium at 6× on this VM, four interleaved runs each (before the bounds check, which saved another 4% in Node), the scale's cpuP95 is 25.4 ms (23.8–29.8) against `main`'s 15.9 (15.1–16.4), cpuP50 10.6 against 5.6; the branch before this pass measured 27.7 against 16.4 in two runs each on the same probe. Waking only what the pans move, instead of every stone and part at each tilt, made it slower and was dropped.

## Still weak (intersections)

- The Honest Scale still costs about 1.6 times `main`'s CPU at the 95th percentile and 1.9 times at the median (above). What is left is cannon's own work on the colliders as drawn: separating-axis tests against the pans' wall boxes and the stones' prisms, and the solver's extra contacts. Fewer, larger pan wall boxes would cut it, but change what the pans are made of and need a fresh audit.
