...
Each item is a mistake made while building Pebble Table.
...
Pebble Table itself has not done three of these yet: no scene has a want (guests look straight ahead at rest, per `gazeTarget` in `games/pebble-table/feeding.ts`), the cold playtest proxy has not been run, and no physical iPad has been measured. See the filled-in checklist under Examples.
