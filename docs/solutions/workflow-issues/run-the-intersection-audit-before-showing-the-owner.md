...
Each game then had its own pass (PRs #19 to #30, and Kite Tower's, Critter Clay's and Pebble Table's, merged from bundles as "Merge cursor/kite-tower-intersections-bundle", "Merge cursor/critter-clay-intersections-bundle" and "Merge cursor/pebble-table-rapier-cceb"): script the moments, fix what is real, allow what is meant, and set `enforce: true` so CI keeps it clean. All twelve three.js games are enforced now: Shadow Lantern, Frog Choir, Moon Phases, Felt Meadow, Light Garden, Hillside Spring, Cosy Scarf, Bedtime Forest, Turning Tower, Kite Tower, Critter Clay and Pebble Table. Bad Neighbours (PR #21) is canvas 2D, which the audit does not read. The tool changed three times from what the passes ran into (PRs #17, #18 and #23).
...
- **A run that is not the same run.** A cap can absorb a contact that goes a little deeper in one run. It cannot absorb a finding that is open in one run and missing from the next, and that turns an enforced game's CI red at random. Pebble Table's enforcing pass was clean twice on the VM, each run 205 samples and 85 pieces (its pass's PR body). Then `Intersection audit (3/4)` failed on the pushed bundle (CI run 36038821759), whose game code was the same, merged with PR #31's docs only. It reported `pebble-table: FAIL - 1 open, 0 allowed, 2 hidden; 205 samples, 87 pieces`, two stones 9% into each other at 6.86 s in the story-and-spill moment (`penetration 9%  stone-whole #c9683d  x  stone-whole #c9683d  [story-spill @ 6.86s]`). The samples were the same, but the pieces were not, so the run itself had changed. Its pass's tool notes had already seen it: "Each physics run tips the jars a few milliseconds differently, so each run can surface a different single finding." The coordinator's handoff notes put the piece count at 83 to 88 between runs, reproducible under CPU load. Pebble Table's final pass (carried into "Merge cursor/pebble-table-rapier-cceb", which also moved its physics from cannon-es to Rapier) found three routes by which machine speed reached the table, each measured with an instrumented build. Shared randomness: the bag's fling and a jar's pour drew from the shared `Math.random`, which sound (clacks rate-limited by the audio clock, audio ready once its real-time `resume()` settles) and three.js (object UUIDs, shader compiles) also drew from at moments set by machine speed; three runs had made 108739, 108759 and 108743 draws by the end of the spill. Frame-length stepping: the controller advanced by each frame's own length, and physics caught up at most three 120 Hz steps a frame, two on the lean and minimal tiers, so frame length and the governor's tier changed where stones landed. The start phase: the audit's zero sits on its 33 ms grid while the game draws on the 16 ms animation-frame grid, and r3f timed the first frame from when the canvas was made, so taps landed a frame or two apart relative to the game. The fixes: the table draws from a seeded stream of its own (`games/pebble-table/random.ts`); `TableController.step` plays whole fixed steps and carries the rest over, cutting only a frame longer than 1/20 s, and quality tiers change drawing only; the first frame plays no time (`games/pebble-table/view/game.tsx`), and the game's audit config runs the page clock to its eighth drawn frame before the story starts (`toFrame` in `scripts/intersections/games/pebble-table.ts`). On Rapier a fourth route opened: the table fetches and starts its WebAssembly physics before it makes its renderer, in real time, so the audit now waits for the renderer in real time with the page's clock paused, for a game whose config says `mountsAfterLoading: true` and only for it (`scripts/intersections/types.ts`). The merged game's refinement log records three enforced audits under ten busy loops printing the same line, `clean: 0 open, 0 allowed, 2 hidden; 205 samples, 85 pieces`, as CI's did (`games/pebble-table/REFINEMENT.md`, "Rapier pass").

  The clock does not carry the load into the game. Under the audit's clock, a probe on Playwright 1.63 found every frame 16.0 ms after the last (61 intervals) and `performance.now()` unmoved inside a frame despite a busy loop there (62 frames). So a game's frame time, and a governor's reading of its CPU work, are the same under any load. Pebble Table reads no `tier` query, so the runner leaves its governor automatic, but under that clock the governor has nothing to step down on. Whatever differs must reach the game through something the paused clock does not drive. Before enforcing, and after any change to physics or timing, run `--ci` at least five times, some of them under CPU load. Samples, pieces and open findings must match in every run: a piece count that moves means the runs are not the same, whatever the findings say. Keep the simulation on game time alone, with a fixed step and a step count each frame that only the game time decides, never the quality tier or how long the frame took; give it a random stream nothing else draws from; and make the first frame play no time. Pebble Table's cap used to follow its tier (`physicsSubsteps` 3, 3, 2 and 2, applied as `maxSubsteps`), and its final pass removed it: `games/pebble-table/determinism.test.ts` now plays the opening spill under frames from 120 Hz to 41 ms of jitter, and at every tier, and gets the same stones bit for bit.

  The start phase reaches every game, not only one that loads physics. Critter Clay's enforced audit failed on the M4 under load in 2 runs of 3 on cannon best (`ae6bb75`) and in 1 of 3 and 1 of 4 on `main`, with the horn on the finger over its socket at 204% into the lump, or at 138% in the clean runs, at the same moment (`dress-the-lump @ 12.01`). The commit that made the real-time wait opt-in (`1128ffa`) credits that wait with moving Critter Clay's timeline; it did not, and the flake predates it. An instrumented build found the game the same in every run: 0.016 s of game time over 2 frames at the audit's zero, and 8.816 s over 552 frames at the end of the first moment. Only where the audit's zero fell on the game's 16 ms frame grid moved (100, 102 and 133 ms after load; the two runs at 133 ms matched), because the game mounts once its code has loaded, in real time, and the runner's first-frame loop stops on its own 33 ms grid wherever that happens to fall. The runner now runs the page's clock on a millisecond at a time to the next drawn frame after the first frames and after every reload, for every game (`onNextFrame` in `scripts/jam-intersections.mjs`). Pebble Table's own `toFrame` does the same job further in. Of the eleven other enforced games, seven had printed different reports across three runs under load before this (every run passing), and ten printed identical ones after; Kite Tower still differs in which of two small contacts it registers late in the run, for a reason not yet found. Two games then sampled a moment on every run that runs had only sometimes sampled before, and each got a reasoned allowance: the part on the finger at 204% in Critter Clay, and the bunny's squash pressing its body 6.6% onto its feet in Cosy Scarf. Critter Clay then printed `clean - 0 open, 101 allowed, 3 hidden; 218 samples, 65 pieces` in five runs of five under ten busy loops, as CI did. `games/critter-clay/determinism.test.ts` covers the game's side: the same frames and touches play the same whatever `Math.random` returns and however far the wall clock runs.
...
- When starting a game's audit pass, or changing a game's physics, timing or randomness.
