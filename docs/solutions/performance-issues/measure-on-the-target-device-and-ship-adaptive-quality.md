...
- [ ] **Cap physics catch-up.** Clamp the accumulator so a slow frame cannot make the next one slower, and keep physics colliders simpler than the drawn meshes (low side counts for cylinders). Cap it above any display frame plus a leftover step (Pebble Table: `LONGEST_FRAME`, six 120 Hz steps), not at 3 steps: at 120 Hz that drops a step from every 30 Hz frame, so the device's speed changes what happens.
