...
applies_when:
  ...
  - Building or refining a react-three-fiber scene with rigid-body physics (Rapier, or cannon-es in an older game) that must hold 60 fps on a mid-range iPad
...
tags: [ipad-performance, adaptive-quality, react-three-fiber, cannon-es, rapier, webkit, cpu-throttling, fps-overlay]
---
...
Pebble Table (`games/pebble-table/`, PR #1) is a claymation 3D math toy (react-three-fiber plus cannon-es at the time; it has run on Rapier since the port in [use Rapier as the default physics engine](../tooling-decisions/use-rapier-as-the-default-physics-engine-for-jam-3d-games.md)) for a 4-year-old on an iPad. Through the whole visual refinement loop, every pass "held 60 fps", and every one of those numbers came from headless Chrome under Playwright on the Apple M4 Mac mini the agent worked on. The owner then played it on a real device and said: "make sure it's not lagging, right now it's lagging a lot."
...
**Cheaper colliders, capped substeps** (`games/pebble-table/physics3d.ts`). The collider half of this fix has since been replaced; the catch-up cap and the calm rule for loose parts still stand:

- On cannon-es, stone colliders dropped from 12 sides to 8 and fixtures such as guests and stools to 10 (`STONE_SIDES`, `FIXTURE_SIDES`, now gone). Convex-convex cost grows with faces times edges, and a spill is nearly all stone-on-stone contacts. That trade no longer holds. Colliders smaller or rounder than the drawing let pieces sink into or float over each other ([pieces collide as drawn](../ui-bugs/pieces-collide-as-drawn-and-rest-on-what-is-drawn.md)), and fitting them to the drawing on cannon-es regressed the Honest Scale badly, which is what moved Pebble Table to Rapier ([use Rapier as the default physics engine](../tooling-decisions/use-rapier-as-the-default-physics-engine-for-jam-3d-games.md)). Now stones collide as convex pieces of their drawn outline (`prismPieces`), round fixtures and guests' arms as true cylinders a hair wider than drawn (`FIXTURE_PROUD`), and loose parts as one hull or a few capsules fitted to their drawings (`games/pebble-table/partShape.ts`). Few shapes per piece still matters: the first Rapier port, with dozens of traced balls per part, cost 1.62 ms a frame on the seeded busy scale in Node, and the hulls and capsules (with the SIMD build) 0.37 ms.
- Catch-up is capped at `LONGEST_FRAME` (1/20 s, six 120 Hz steps), so an overloaded frame slows game time instead of spiralling. The cap once was 3 steps and followed the quality tier (2 on lean and minimal), which dropped a step from every paced 33 ms frame and made what happened depend on the device and the tier. Now `TableController.step` plays each frame as whole `STEP`s with the rest carried over, and only a frame longer than any display frame loses time; tiers change only how the table is drawn. `games/pebble-table/determinism.test.ts` plays the opening spill under frames from 120 Hz to 41 ms of jitter, and at every tier, and gets the same stones bit for bit.
- Resting bodies must fall asleep. The jars of loose parts (stacked branch `cursor/pebble-table-explore-cceb`) first modelled shells as thin cones (`Cylinder(2, 1.65, 0.7)`, mass 1). Six of them piled together kept nudging each other awake, so in 2 of 8 headless trials some parts were still moving 20 s after the tip and physics cost about 1 ms a step on the M4 indefinitely. Flat discs of equal radii plus firmer sleep settings (`angularDamping: 0.9`, `sleepSpeedLimit: 2`, `sleepTimeLimit: 0.3` in `addPart`) settled most trials, with steps under 0.2 ms, and took the scene from 21 to 49 fps at 20x. But the controller test "lets every tipped-out part come to rest" still failed about once in 13 runs: shells or sticks stacked on each other kept bouncing at up to 6 units/s, the convex-on-convex stacking jitter cannon is known for. Two more changes made it reliable (0 failures in 320 trials). Shells and sticks collided as clusters of small spheres, which stacked stably on cannon-es. And a calm timer put any loose part to sleep once it had moved slower than 4 units/s for a second, so parts nudging each other just above cannon's sleep limit still went quiet. On Rapier the sphere clusters became hulls and capsules and cannon's sleep settings are gone, but the calm timer stays. Once a loose body has been slower than `LOOSE_CALM_SPEED` (4 units/s) for `LOOSE_CALM_SECONDS` (1 s), `settleLoose` counts it as asleep and holds it still every step so Rapier's own sleeping takes it and its pile; a step that leaves it moving faster makes it loose again. Bodies are never put to sleep by hand, and a moving body wakes what it could reach before it gets there (`wakeAhead`; see [wake only what a moving body can reach](wake-only-what-a-moving-body-can-reach-and-let-the-mover-stop.md)). Run a new rest test in a loop (dozens of runs) before trusting it; a single green run proved nothing here.
...
| Tier | DPR cap | Fur shells | Post |
|---|---|---|---|
| full | 2 | 6 | tilt-shift blur plus grade |
| balanced | 1.5 | 3 | tilt-shift blur plus grade |
| lean | 1.25 | 0 | grade and vignette only |
| minimal | 1 | 0 | off; grade runs in materials via `CustomToneMapping` |
...
**Frame-budget test in CI** (`games/pebble-table/perf.test.ts`). It spills ten stones from the bag through the real `TableController`, times 180 frames of `step` in each of seven replays, takes each frame's minimum across the replays, and fails if the average of those minimums exceeds 0.75 ms. A busy runner stalls random frames, not the same frame in every replay, so the per-frame minimum is the real cost. The first version asserted the best of five runs' averages, and that failed on a shared CI runner. It runs in `npm test`, which CI runs on every push. The budget is loose enough not to flake on a busy runner and tight enough to catch a collider or substep regression, which costs several times that. A second test in the same file counts the Honest Scale's physics work at its busiest (steps, awake bodies, and solver contacts a frame) and asserts that nothing is left awake once the table settles. How to write one that holds on a shared runner (count the work when you can, otherwise per-frame minimums over aligned replays) is in [frame-budget tests that hold on a shared CI runner](../test-failures/frame-budget-tests-that-hold-on-a-shared-ci-runner.md).
...
2. **Step down by counting missed frames in short windows, not by averaging intervals.** An average hides steady judder and magnifies one hiccup. Four frames at 16.7 ms and one at 33.3 average 20 ms, under the 21 to 22 ms thresholds of the averaging governors, while visibly stuttering at 48 fps. Fed that pattern from the top tier, Pebble Table, Hillside Spring, and Kite Tower (a window is bad when more than 10% of 40 frames run over 20 ms) stepped down, and so did Frog Choir, whose 19.5 ms average threshold sits just under 20; the other seven stayed at full quality. Bedtime Forest and Critter Clay count too, but only call a window slow at a quarter and a half of its frames. The other side: Felt Meadow's one-second averages stepped down on a single frame of 260 ms or more (200 ms did not; 900 ms dropped two tiers), and a second failure at a tier made it a ceiling for the session, so a 300 ms hiccup every 10 s walked it from full quality to its lowest tier within a minute. None of the other ten moved on the same hiccups. A follow-up merge on `main` made Felt Meadow leave each window's longest frame out, as Pebble Table does (`games/felt-meadow/perf.ts`); its test feeds one 300 ms frame (no change) and two (one tier down). Close windows on time as well as frame count, so a very slow device is judged in seconds: at 400 ms frames Pebble Table and Hillside Spring, with 40-frame windows only, took 32 s to take a first step, while Kite Tower (40 frames or 2 s) and Turning Tower (90 frames or 1.5 s) took 4 and 6.4 s. Keep stalls at 1 s, not 250 ms: Kite Tower pass 0 and Turning Tower pass 16 found the 250 ms filter this doc records, and Cosy Scarf ignored every interval over 250 ms, so at 400 ms frames it never left the top tier. A follow-up merge on `main` moved its `IGNORE_OVER_MS` to 1000 and made its drops skip a tier when far off the pace; at 400 ms frames its test now reaches the lowest tier, two tiers at a time. Frog Choir keeps a 250 ms gap filter but counts three long gaps in a row as slow frames, which works.
3. **Let a very slow second drop two tiers.** Bedtime Forest (three quarters of a half-second window over 80 ms), Frog Choir (a one-second average over 40 ms), Felt Meadow (over 34 ms), and, since the follow-up merge, Cosy Scarf (a smoothed interval over 34 ms) drop two tiers at once. At 120 ms frames they took their first step within 1.1 to 2.3 s and landed two tiers down; the one-tier governors took 1.7 to 9.6 s for a first step, and Pebble Table and Hillside Spring were still a tier above their lowest after 20 s.
4. **Start touch devices one tier down.** The governor needs seconds to learn, and those are the child's first seconds. All eleven governors now start a coarse pointer one tier down. Bedtime Forest and Turning Tower started every device at full quality until a follow-up merge on `main` gave them `startingTier(coarsePointer)`, like the others (`games/bedtime-forest/perf.ts`, `games/turning-tower/tiers.ts`); both step back up on 60 Hz frames, which their tests check. Playwright's `hasTouch` matches `(pointer: coarse)` in both engines, so a probe with touch on measures the touch start.
...
- [ ] **Add a frame-budget test to CI** that drives the heaviest simulation through the real controller. Count the work that sets the cost when the code exposes it (steps, contacts, scored candidates); otherwise take each frame's minimum across seeded replays that line up frame for frame (Pebble Table: the average of per-frame minimums across seven ten-stone spills under 0.75 ms). Assert that the heavy moment happened, and never assert a raw maximum. See [frame-budget tests that hold on a shared CI runner](../test-failures/frame-budget-tests-that-hold-on-a-shared-ci-runner.md).
...
- [`games/pebble-table/ART.md`](../../../games/pebble-table/ART.md): adaptive quality, the grown-up overlay, and colliders as built.
- [`use-rapier-as-the-default-physics-engine-for-jam-3d-games.md`](../tooling-decisions/use-rapier-as-the-default-physics-engine-for-jam-3d-games.md): why Pebble Table left cannon-es, and how its colliders, sleeping, and catch-up work on Rapier.
- [`pieces-collide-as-drawn-and-rest-on-what-is-drawn.md`](../ui-bugs/pieces-collide-as-drawn-and-rest-on-what-is-drawn.md): why colliders now match the drawing instead of undercutting it.
