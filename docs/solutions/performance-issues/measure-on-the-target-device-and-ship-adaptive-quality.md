...
- Resting bodies must fall asleep. The jars of loose parts (stacked branch `cursor/pebble-table-explore-cceb`) first modelled shells as thin cones (`Cylinder(2, 1.65, 0.7)`, mass 1). Six of them piled together kept nudging each other awake, so in 2 of 8 headless trials some parts were still moving 20 s after the tip and physics cost about 1 ms a step on the M4 indefinitely. Flat discs of equal radii (`Cylinder(1.85, 1.85, 0.9)`) and firmer sleep settings for parts (`angularDamping: 0.9`, `sleepSpeedLimit: 2`, `sleepTimeLimit: 0.3` in `addPart`) made every trial settle within about 6 s, with steps under 0.2 ms, and the scene went from 21 to 49 fps at 20x. The controller test "lets every tipped-out part come to rest" keeps it that way.
